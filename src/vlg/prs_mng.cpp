...
RetCode persistence_manager_impl::load_pers_driver_dyna(const char *drivers[],
                                                        int drivers_num)
{
    IFLOG(get_instance().log_, trace(LS_OPN "[drivers_num:{}]", __func__, drivers_num))
    RetCode rcode = RetCode_OK;
    for(int i = 0; i < drivers_num; i++) {
        if(get_instance().drivname_driv_hm_.find(drivers[i]) != get_instance().drivname_driv_hm_.end()) {
            IFLOG(get_instance().log_, warn(LS_TRL "[driver already loaded, skipping:{}]", __func__, drivers[i]))
            continue;
        }
        persistence_driver *driv = nullptr;
        if((rcode = persistence_driver::load_driver_dyna(drivers[i], &driv, get_instance().log_))) {
            IFLOG(get_instance().log_, critical(LS_CLO "[failed loading driver:{}][res:{}]", __func__, drivers[i], rcode))
            return rcode;
        }
...
{
    IFLOG(get_instance().log_, trace(LS_OPN "[drivers_num:{}]", __func__, drivmap.size()))
    RetCode rcode = RetCode_OK;
    for(auto it = drivmap.begin(); it != drivmap.end(); it++) {
        persistence_driver *driv = nullptr;
        if((rcode = persistence_driver::load_driver_dyna(it->c_str(), &driv, get_instance().log_))) {
            IFLOG(get_instance().log_, critical(LS_CLO "[failed loading driver:{}][res:{}]", __func__, it->c_str(), rcode))
            return rcode;
        }
                                                          ...
                                                          int drivers_num)
{
    ...
    for(int i = 0; i < drivers_num; i++) {
        if(get_instance().drivname_driv_hm_.find(drivers[i]->get_driver_name()) != get_instance().drivname_driv_hm_.end()) {
            IFLOG(get_instance().log_, warn(LS_TRL "[driver already loaded, skipping:{}]", __func__, drivers[i]->get_driver_name()))
            continue;
        }
...
RetCode persistence_manager_impl::start_all_drivers()
{
    ...
    for(auto it = drivname_driv_hm_.begin(); it != drivname_driv_hm_.end(); it++) {
        if((rcode = it->second->start_all_pools())) {
            IFLOG(log_, critical(LS_TRL "[failed to start persistence-driver:{}]", __func__, it->first.c_str()))
            break;
        }
    }
...
persistence_driver *persistence_manager_impl::available_driver(unsigned int nclass_id)
{
    ...
    if(it == nclassid_driv_hm_.end()) {
        IFLOG(log_, error(LS_TRL "[nclass_id:{} has no persistence-driver available]", __func__, nclass_id))
    }
    return it->second;
}
                                            ...
                                            std::string &psswd)
{
    ...
    while(tknz.next_token(tkn, DF_DLM CLN, true)) {
        ...
        if(tkn == VLG_RWRD_URL) {
            ...
            IFLOG(log_, critical(LS_PRS "[line:{}, unexpected token:{}]", __func__, lnum, tkn.c_str()))
            return RetCode_BADCFG;
        }
    }
    //read mandatory colon
                                                             ...
                                                             std::unordered_map<std::string, persistence_driver *> &conn_pool_name_to_driv)
{
    ...
    while(tknz.next_token(tkn, DF_DLM, true)) {
        ...
        if(it == drivname_driv_hm_.end()) {
            IFLOG(log_, critical(LS_PRS "[line:{}, driver not loaded:{}]", __func__, lnum, tkn.c_str()))
            return RetCode_BADCFG;
        } else {
            driv = it->second;
        }
                                                              ...
                                                              std::unordered_map<std::string, persistence_driver *> &conn_pool_name_to_driv)
{
    ...
    while(tknz.next_token(tkn, DF_DLM, true)) {
        ...
        if(it == conn_pool_name_to_driv.end()) {
            IFLOG(log_, critical(LS_PRS "[line:{}, connection pool not defined:{}]", __func__, lnum, tkn.c_str()))
            return RetCode_BADCFG;
        } else {
            driv = it->second;
        }
    ...
    while(tknz.next_token(tkn, DF_DLM, true)) {
        ...
        if(tkn == VLG_RWRD_MAP_CFG_END) {
            ...
            if(!string_is_number(nclass_id.c_str())) {
                IFLOG(log_, critical(LS_PRS "[line:{}, bad nclass_id:{}]", __func__, lnum, nclass_id.c_str()))
                return RetCode_BADCFG;
            }
...
{
    IFLOG(log_, trace(LS_OPN "[filename:{}]", __func__, filename))
    std::string path;
    ...
    if(!fdesc) {
        IFLOG(log_, warn(LS_CLO "[cannot open persistent configuration file:{}]", __func__, filename))
