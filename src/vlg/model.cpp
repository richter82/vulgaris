...
struct nentity_manager_impl {
    ...
    RetCode extend(const nentity_desc &nent_desc) {
        ...
        IFLOG(log_, trace(LS_CLO"[num_nenum:%d, num_nclass:%d, num_nentity:%d]", __func__,
    ...
    RetCode extend(const char *model_name) {
        IFLOG(log_, trace(LS_OPN "[model_name:%s]", __func__, model_name))
...
#endif
        if(!dynalib) {
            IFLOG(log_, error(LS_CLO "[failed loading so-lib for model:%s]", __func__, model_name))
            return RetCode_KO;
        }
        ...
        if(!nem_f) {
            IFLOG(log_, error(LS_CLO "[failed to locate nem entrypoint in so-lib for model:%s]", __func__, model_name))
            return RetCode_KO;
        }
        ...
        IFLOG(log_, info(LS_MDL"model:%s [loaded]", __func__, mdlv_f()))
        return RetCode_OK;
    }
                             ...
                             void *ud)
{
    ...
    if(kdsc.is_primary()) {
        for(auto it = kdsc.impl_->fieldset_.begin(); it != kdsc.impl_->fieldset_.end(); ++it) {
            const char *obj_f_ptr = nullptr;
            obj_f_ptr = rud->obj_ptr_ + (*it)->get_field_offset();
            FillStr_FldValue(obj_f_ptr, (*it)->get_field_vlg_type(), (*it)->get_field_nmemb(), rud->out_);
        }
        return false;
    } else {
        return true;
