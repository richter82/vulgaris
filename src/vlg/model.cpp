...
struct nentity_manager_impl {
    ...
    RetCode extend(const nentity_desc &nent_desc) {
        ...
        IFLOG(log_, trace(LS_CLO"[num_nenum:{}, num_nclass:{}, num_nentity:{}]", __func__,
    ...
    RetCode extend(const char *model_name) {
        IFLOG(log_, trace(LS_OPN "[model_name:{}]", __func__, model_name))
...
#endif
        if(!dynalib) {
            IFLOG(log_, error(LS_CLO "[failed loading so-lib for model:{}]", __func__, model_name))
            return RetCode_KO;
        }
        ...
        if(!nem_f) {
            IFLOG(log_, error(LS_CLO "[failed to locate nem entrypoint in so-lib for model:{}]", __func__, model_name))
            return RetCode_KO;
        }
        ...
        IFLOG(log_, info(LS_MDL"model:{} [loaded]", __func__, mdlv_f()))
        return RetCode_OK;
    }
