...
struct nentity_desc_impl {
    ...
    RetCode add_member_desc(const member_desc &mmbrdesc) {
        ...
        const member_desc *mptr = &mmbrdesc;
...
vlg::nclass_alloc nentity_desc::get_nclass_allocation_function() const
{
    return impl_->afun_;
}
...
struct nentity_manager_impl {
    ...
    RetCode extend(const char *model_name) {
        ...
        nentity_manager_func nem_f = (nentity_manager_func)vlg::dynamic_lib_load_symbol(dynalib, nem_ep_f);
        if(!nem_f) {
        ...
        model_version_func mdlv_f = (model_version_func)vlg::dynamic_lib_load_symbol(dynalib, mdlv_f_n);
