...
struct pers_conn_sqlite : public persistence_connection_impl {
    ...
    private:
        ...
        struct persistence_task_sqlite : public persistence_task {
            ...
            protected:
                virtual RetCode do_connect() override {
                    IFLOG(log_, trace(LS_OPN "[url:{}, user:{}, password:{}]", __func__,
                ...
                virtual RetCode do_create_table() override {
                    ...
                    if((rcode = sql_conn_.sqlite_exec_stmt(stmt_bf_, false))) {
                        if(in_drop_if_exist_) {
                            ...
                            drop_stmt.append(";");
                            IFLOG(log_, debug(LS_STM "[drop_stmt:{}]", __func__, drop_stmt.c_str()))
                            if((rcode = sql_conn_.sqlite_exec_stmt(drop_stmt.c_str()))) {
                                IFLOG(log_, error(LS_CLO "[drop failed][res:{}].", __func__, rcode))
                                return rcode;
                            }
                            if((rcode = sql_conn_.sqlite_exec_stmt(stmt_bf_))) {
                                IFLOG(log_, error(LS_CLO "[create after drop failed][res:{}]", __func__, rcode))
                                return rcode;
                            }
                        } else {
                            IFLOG(log_, error(LS_CLO "[create failed][res:{}]", __func__, rcode))
                            return rcode;
                        }
                    }
                    return rcode;
                ...
                virtual RetCode do_select() override {
                    ...
                    if((rcode = sql_conn_.sqlite_prepare_stmt(stmt_bf_, &stmt))) {
                        IFLOG(log_, error(LS_CLO "[sqlite_prepare_stmt failed][res:{}]", __func__, rcode))
                    } else {
                        //bind
                        if((rcode = sql_conn_.sqlite_bind_where_clause(in_key_, 1, *in_out_obj_, stmt))) {
                            IFLOG(log_, error(LS_CLO "[sqlite_bind_where_clause failed][res:{}]", __func__, rcode))
                            RetCode rels_rcode = RetCode_OK;
                            if((rels_rcode = sql_conn_.sqlite_release_stmt(stmt))) {
                                IFLOG(log_, error(LS_TRL "[res:{}][sqlite_release_stmt failed]", __func__, rels_rcode))
                            }
                            ...
                            if(sqlite_rc == SQLITE_ROW) {
                                ...
                                IFLOG(log_, error(LS_TRL "[rc:{}][sqlite_step_stmt: unhandled sqlite code]", __func__, sqlite_rc))
                                rcode = RetCode_DBERR;
                            }
                            RetCode rels_rcode = RetCode_OK;
                            if((rels_rcode = sql_conn_.sqlite_release_stmt(stmt))) {
                            ...
                            IFLOG(log_, error(LS_TRL "[res:{}][sqlite_step_stmt failed]", __func__, rcode))
                        }
                    }
                    return rcode;
                }
                ...
                virtual RetCode do_insert() override {
                    ...
                    if((rcode = sql_conn_.sqlite_prepare_stmt(stmt_bf_, &stmt))) {
                        ...
                        if((rcode = sql_conn_.sqlite_bind_obj_fields(*in_out_ts0_, *in_out_ts1_, *in_obj_, *in_nem_, stmt))) {
                            IFLOG(log_, error(LS_CLO "[sqlite_bind_obj_fields failed][res:{}]", __func__, rcode))
                            RetCode rels_rcode = RetCode_OK;
                            if((rels_rcode = sql_conn_.sqlite_release_stmt(stmt))) {
                                IFLOG(log_, error(LS_TRL "[res:{}][sqlite_release_stmt failed]", __func__, rels_rcode))
                            }
...
{
    IFLOG(log_, trace(LS_OPN "[filename:{}, flags:{}]", __func__, filename, flags))
    RetCode rcode = RetCode_OK;
    std::string db_path;
    ...
    if(last_rc) {
        IFLOG(log_, error(LS_TRL"[filename:{}][sqlite3_open_v2 - rc:{} - errdesc:{}]",
...
inline RetCode pers_conn_sqlite::sqlite_disconnect()
{
    ...
    if(last_rc) {
        IFLOG(log_, error(LS_CLO "[sqlite3_close_v2(rc:{}) - errdesc[{}] - db error]",
...
inline RetCode pers_conn_sqlite::sqlite_exec_stmt(const char *stmt, bool fail_is_error)
{
    ...
    if(last_rc != SQLITE_OK) {
        if(fail_is_error) {
            IFLOG(log_, error(LS_TRL"[sqlite3_exec - rc:{} - errdesc:{}]", __func__, last_rc, zErrMsg))
        } else {
            IFLOG(log_, debug(LS_TRL"[sqlite3_exec - rc:{} - errdesc:{}]", __func__, last_rc, zErrMsg))
        }
...
inline RetCode pers_conn_sqlite::sqlite_prepare_stmt(const char *sql_stmt, sqlite3_stmt **stmt)
{
    ...
    if(last_rc != SQLITE_OK) {
        IFLOG(log_, error(LS_TRL "[sqlite3_prepare_v2 rc:{} - errdesc:{}]", __func__, last_rc, sqlite3_errstr(last_rc)))
        rcode = RetCode_DBERR;
    }
    return rcode;
}
...
bool enum_keyset_bind_where_clause(const member_desc &mmbrd, void *ud)
{
    ...
    if(mmbrd.get_field_vlg_type() == Type_ENTITY) {
        ...
        bind_stmt_fld(mmbrd, obj_f_ptr, kbwc.col_idx++, kbwc.stmt);
    }
    return true;
}
...
inline RetCode pers_conn_sqlite::sqlite_step_stmt(sqlite3_stmt *stmt, int &sqlite_rc)
{
    ...
    if(sqlite_rc != SQLITE_ROW && sqlite_rc != SQLITE_DONE) {
        IFLOG(log_, info(LS_TRL "[sqlite3_step rc:{} - errdesc:{}]", __func__, sqlite_rc, sqlite3_errstr(sqlite_rc)))
        rcode = RetCode_DBERR;
    }
    return rcode;
}
...
inline RetCode pers_conn_sqlite::sqlite_release_stmt(sqlite3_stmt *stmt)
{
    ...
    if(last_rc != SQLITE_OK) {
        IFLOG(log_, error(LS_TRL "[sqlite3_finalize rc:{}]", __func__, last_rc))
        rcode = RetCode_DBERR;
    }
    return rcode;
}
                                          ...
                                          bool drop_if_exist)
{
    ...
    create_stmt.append(");");
    std::unique_ptr<persistence_task_sqlite> task(new persistence_task_sqlite(*this, VLG_PERS_TASK_OP_CREATETABLE, log_));
    task->in_edesc_ = &edesc;
    task->in_drop_if_exist_ = drop_if_exist;
    task->stmt_bf_ = create_stmt.c_str();
    IFLOG(log_, debug(LS_STM "[create_stmt:{}]", __func__, create_stmt.c_str()))
    if((rcode = worker_->submit(*task))) {
        return rcode;
    } else {
        task->await_for_status(PTskStatus_EXECUTED);
                                    ...
                                    nclass &in_out)
{
    ...
    std::unique_ptr<persistence_task_sqlite> task(new persistence_task_sqlite(*this, VLG_PERS_TASK_OP_SELECT, log_));
    task->in_nem_ = &nem;
    task->in_key_ = key;
    ...
    IFLOG(log_, trace(LS_QRY "[select_stmt:{}]", __func__, sel_stmt))
    if((rcode = worker_->submit(*task))) {
        return rcode;
    } else {
        task->await_for_status(PTskStatus_EXECUTED);
                                    ...
                                    const nclass &in)
{
    ...
    std::unique_ptr<persistence_task_sqlite> task(new persistence_task_sqlite(*this, VLG_PERS_TASK_OP_UPDATE, log_));
    task->in_nem_ = &nem;
    task->in_key_ = key;
    task->in_obj_ = &in;
    ...
    IFLOG(log_, debug(LS_STM "[update_stmt:{}]", __func__, upd_stmt))
    if((rcode = worker_->submit(*task))) {
        return rcode;
    } else {
        task->await_for_status(PTskStatus_EXECUTED);
                                    ...
                                    const nclass &in)
{
    ...
    std::unique_ptr<persistence_task_sqlite> task(new persistence_task_sqlite(*this, VLG_PERS_TASK_OP_DELETE, log_));
    task->in_nem_ = &nem;
    task->in_key_ = key;
    task->in_out_ts0_ = &ts_0;
    task->in_out_ts1_ = &ts_1;
    ...
    IFLOG(log_, debug(LS_STM "[delete_stmt:{}]", __func__, del_stmt))
    if((rcode = worker_->submit(*task))) {
        return rcode;
    } else {
        task->await_for_status(PTskStatus_EXECUTED);
                                    ...
                                    bool fail_is_error)
{
    ...
    std::unique_ptr<persistence_task_sqlite> task(new persistence_task_sqlite(*this, VLG_PERS_TASK_OP_INSERT, log_));
    task->in_nem_ = &nem;
    task->in_obj_ = &in;
    task->in_out_ts0_ = &ts_0;
    task->in_out_ts1_ = &ts_1;
    ...
    IFLOG(log_, debug(LS_STM "[insert_stmt:{}]", __func__, ins_stmt))
    if((rcode = worker_->submit(*task))) {
        return rcode;
    } else {
        task->await_for_status(PTskStatus_EXECUTED);
...
{
    RetCode rcode = RetCode_OK;
    IFLOG(log_, trace(LS_QRY "[query-sql:{}]", __func__, sql))
...
{
    RetCode rcode = RetCode_OK;
    std::unique_ptr<persistence_task_sqlite> task(new persistence_task_sqlite(*this, VLG_PERS_TASK_OP_RELEASEQUERY, log_));
...
{
    RetCode rcode = RetCode_OK;
    IFLOG(log_, trace(LS_STM "[sql:{}]", __func__, sql))
                                         ...
                                         persistence_connection_impl **new_conn)
{
    ...
    *new_conn = new_conn_instance;
    IFLOG(log_, trace(LS_CLO "[id:{}]", __func__, new_conn_instance->id_))
    return rcode;
}
...
RetCode pers_driv_sqlite::close_connection(persistence_connection_impl &conn)
{
    ...
    IFLOG(log_, trace(LS_CLO "[id:{}]", __func__, conn.id_))
    return res;
}
