...
// #VER#
//-----------------------------
#define BLZ_COMP_STRT_STR "blaze.compiler.ver.%d.%d.%d.%d.date:" __DATE__
int comp_ver[4]  = {0,0,0,0};

blaze::config_loader conf_ldr;
...
int main(int argc, char *argv[])
{
    ...
    if(!strcmp("-file", argv[1])) {
        COMMAND_IF_NOT_OK(blaze::conf_ldr.init("params"),
                          EXIT_ACTION_STDOUT("reading parameters file" COMPILE_FAIL))
    } else {
        COMMAND_IF_NOT_OK(blaze::conf_ldr.init(argc, argv),
                          EXIT_ACTION_STDOUT("reading parameters" COMPILE_FAIL))
    }
    COMMAND_IF_NOT_OK(blaze::comp_cfg.path_list.push_back("."), exit(1))
    COMMAND_IF_NOT_OK(blaze::conf_ldr.load_config(),
                      EXIT_ACTION_STDOUT("loading configuration" COMPILE_FAIL))
    blaze::conf_ldr.dump_config();
    blaze::conf_ldr.enum_params(blaze::blz_comp_param_clbk);
    char fname[CR_MAX_SRC_FILE_NAME_LEN];
    blaze::comp_cfg.file_list.start_iteration();
    while(!blaze::comp_cfg.file_list.next(fname)) {
        blaze::compile_unit cmpl_unit;
